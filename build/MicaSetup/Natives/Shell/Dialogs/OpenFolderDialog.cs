...
public sealed class OpenFolderDialog
{
    ...
    /// <returns>A <see cref="DialogResult"/> indicating whether the user clicked OK, Cancel, or if the operation was aborted.</returns>
    public DialogResult ShowDialog(nint owner) // IWin32Window
    {
        var hwndOwner = owner != IntPtr.Zero ? owner : NativeImports.GetActiveWindow();
        ...
        if (Multiselect)
        {
            ...
            for (uint i = 0; i < count; i++)
            {
                ...
                shellItem.GetDisplayName(SIGDN.SIGDN_FILESYSPATH, out var itemPath);
                paths[i] = itemPath;
            }

            SelectedPaths = paths;
            SelectedPath = count > 0 ? paths[0] : null;
        }
        else
        {
            ...
            SelectedPath = path;
            SelectedPaths = path is not null ? [path] : null;
        }

        return DialogResult.OK;
    }

    /// <summary>Gets or sets a value indicating whether the user can select multiple folders.</summary>
    public bool Multiselect { get; set; }

    /// <summary>Gets the paths of the folders selected by the user when <see cref="Multiselect"/> is <see langword="true"/>. This property is populated after <see cref="ShowDialog()"/> returns <see cref="DialogResult.OK"/>.</summary>
    ...
    /// <summary>Gets the path of the folder selected by the user. When <see cref="Multiselect"/> is <see langword="true"/>, this returns the first selected path. This property is populated after <see cref="ShowDialog()"/> returns <see cref="DialogResult.OK"/>.</summary>
    ...
    private void Configure(IFileOpenDialog dialog)
    {
        ...
        {
            var iidGuid = typeof(IShellItem).GUID;
            var result = NativeImports.SHCreateItemFromParsingName(InitialDirectory!, IntPtr.Zero, ref iidGuid, out var shellItemPtr);
            switch ((int)result)
            {
    ...
    private FOS CreateOptions()
    {
        ...
        if (Multiselect)
        {
            result |= FOS.FOS_ALLOWMULTISELECT;
        }
